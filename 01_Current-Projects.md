...
### 運用上の習慣と担当
- 貯める（担当: その会話をした AI 各自）: 会話の節目で更新案を作り、PR にする。
- 整理する（担当: Claude Code）: 月1回ほど棚卸しする（重複の統合・古い情報の削除・見出し整理・不要ブランチの削除）。貯めるだけだとノイズが増えて使いにくくなるため。
- 起動と最終マージ承認（担当: 人間）: 月次整理の起動と、各 PR のマージ判断をおこなう。
...
- 月次の棚卸しを自動ルーティーン化する設定（/schedule）は、claude.ai リモート接続エラーで保留中。後日あらためて /schedule を再試行する。それまでは人間がリマインドして起動する。
